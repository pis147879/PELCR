...
SendAddEdge(edge *N, edge *E, char *sw, int sto) {
	...
	if (N->rankpuit == rank) {
		...
		DEBUG_DISTRIBUTION {
			...
			fprintf(logfile, "%p)]->[%p] Reference: [(%d),(%d,", (void *)N->source, (void *)target, E->rankpuit,
			        E->creator);
		...
		if (size != 1) {
			{
				...
				while (localh != h) {
					...
					if (flag) {
...
#endif
						AddEdge((node *)pS, raddbuf[3], (node *)(long)raddbuf[4], raddbuf[5], swgt, raddbuf[6], LEFT,
						        LEFT);
						...
						DEBUG_MEM {
							...
							for (; P != NULL;) {
								...
								for (; e != NULL; e = e->vector)
									fprintf(logfile, "[%p] %s ", e->weight, e->weight);
								fprintf(logfile, "\n");
								P = P->nextpuit;
							};
						};
						...
						addflag = 0;
						h = h + 1;
						MPI_Irecv(raddbuf, MAXLENWEIGHT, MPI_INT, MPI_ANY_SOURCE, ADD_TAG, MPI_COMM_WORLD,
						          &add_request);
					} else
						unaddtest++;
#ifdef _DEBUG
...
void
SendCreateNewNode(int dest, int sto, edge *nS) {
	if ((dest == rank) && (sto == IN)) {
...
void
PopMessage(struct messaggio *m, struct mbuffer *l) {
	int i;
		...
		if (m->funWhich == EXT) {
			...
			switch (m->funWhich) {
				case SUCC:
					...
					break;
				case ADD:
					f[indice].fun = pelcr_add;
					break;
				case PRED:
					f[indice].fun = pelcr_pred;
					break;
				case MULT:
					f[indice].fun = pelcr_mult;
					break;
				case AND:
					f[indice].fun = pelcr_and;
					break;
				case NOT:
					f[indice].fun = pelcr_not;
					break;
				case ISZERO:
					f[indice].fun = pelcr_iszero;
					break;
			}
		}
