...
int
product(char *a, char *b, char *a1, char *b1) {
...
//	printf("B5:  %s %s\n",btmp,b1);
				...
				checkone(a1);
				checkone(b1);
				if ((propagaInv == 30) || (propagaInv == 31) || (propagaInv == 32) || (propagaInv == 33))
					return propagaInv;
				else
					return 5;
			} else
	...
	/***************** Algebra esterna ********************/
		...
		if ((i1 == TYPE) && (i2 == ARG)) {
			...
			if (product(atmp, btmp, a1, b1)) {
				char ax[100];
				if (t[l1][1] == TIN)
					sprintf(ax, "X(%d,%ld)%s", SYNC, l2 - 1, a1);
				else
