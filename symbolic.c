...
int product(a,b,a1,b1)
 ...
 char*b1;
{
  char tmp[MAXLENWEIGHT];
  long l1,l2,i1,i2,p1,p2;
  char*r;
  char*atmp=a,*btmp=b;
  if(atmp[0]=='\0') {
    memcpy(tmp,b1,MAXLENWEIGHT*sizeof(char));
    if(-1==snprintf(b1,MAXLENWEIGHT,"%s%s",tmp,btmp)){
...
//   printf("B1:  %s %s\n",btmp,b1);
  ...
  /*************************************/
    ...
    memcpy(tmp,a1,MAXLENWEIGHT*sizeof(char));
    if(-1 == snprintf(a1,MAXLENWEIGHT,"%s%s",tmp,atmp)){
	    fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
    }
#ifdef WDEBUG
//    printf("A2 %s %s\n",atmp,a1);
...
//	printf("B5:  %s %s\n",btmp,b1);
#endif
	{
	  char ax[MAXLENWEIGHT];
	  char bx[MAXLENWEIGHT];
	  //char ax[100];
	  //char bx[100];
	  if(-1 == snprintf(ax,MAXLENWEIGHT,"%s",a1)){
		  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
	  }
	  if(-1 == snprintf(bx,MAXLENWEIGHT,"%s",b1)){
		  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
	  }
	  if(-1 == snprintf(a1,MAXLENWEIGHT,"!%ld%s",p1,ax)){
		  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
	  }
	  if(-1 == snprintf(b1,MAXLENWEIGHT,"!%ld%s",p1,bx)){
		  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
	  }
	}
	checkone(a1);
	...
	if ( (propagaInv==30)||(propagaInv==31) ||(propagaInv==32)||(propagaInv==33)) return propagaInv;
	    ...
	    //char ax[100];
	    //char bx[100];
	    if(-1 == snprintf(bx,MAXLENWEIGHT,"!%ld%s",p2-p1,btmp)){
		    fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
	    }
	    ...
	    //char ax[100];
	    //char bx[100];
	    if(-1 == snprintf(ax,MAXLENWEIGHT,"!%ld%s",p1-p2,atmp)){
		    fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
	    }
	    if(product(ax,btmp,a1,b1)) {
		    ...
		    if(-1 == snprintf(a1,MAXLENWEIGHT,"!%ld%s",p2,ax)){
			    fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
		    }
		    if(-1 == snprintf(b1,MAXLENWEIGHT,"!%ld%s",p2,bx)){
			    fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
		    }
		    checkone(a1);checkone(b1);
		    return 7;
  ...
  /*************************************/
	  ...
	  px= p1+l2-1;
	  if(px!=0){
		  if(-1 == snprintf(ax,MAXLENWEIGHT,"!%ld%s",px,atmp)){ 
			  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
		  }
	  }
	  else{
		  if(-1 == snprintf(ax,MAXLENWEIGHT,"%s",atmp)){
			  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
		  }
	  }
	  if(product(ax,btmp,a1,b1)) {
		  ...
		  if(-1 == snprintf(b1,MAXLENWEIGHT,"w(%ld,%ld)%s",i2,l2,bx)){
			  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
		  }
	  }
	  else return 0;
  ...
  /*************************************/
	  ...
	  px= p2+l1-1;
	  if(px!=0){
		  if(-1 == snprintf(bx,MAXLENWEIGHT,"!%ld%s",px,btmp)){
			  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
		  }
	  }else{ 
		  if(-1 == snprintf(bx,MAXLENWEIGHT,"%s",btmp)){
			  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
		  }
	  }
	  if(product(atmp,bx,a1,b1)) {
		  ...
		  if(-1 == snprintf(a1,MAXLENWEIGHT,"w(%ld,%ld)%s",i1,l1,ax)){
			  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
		  }
	  }
	  else return 0;
  ...
  /***************** Algebra esterna ********************/
	  ...
	  btmp= r+1;
#ifdef WDEBUG
	  printf("Extern Algebra --> branch: %ld , %ld\n",i1,i2);
#endif
	  l1 = strtol(atmp,&r,10);
	  atmp = r+1;
	  l2 = strtol(btmp,&r,10);
	  ...
	  if ( (i1==TYPE)&&(i2==ARG) ) {     
		  ...
		  if (product(atmp,btmp,a1,b1)) {
			  char ax[100];  
			  if(t[l1][1]==TIN){
				  if(-1 == snprintf(ax,MAXLENWEIGHT,"X(%d,%ld)%s",SYNC,l2-1,a1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
			  }else{ 
				  if(-1 == snprintf(ax,MAXLENWEIGHT,"X(%d,%ld)%s",SYNC,l2,a1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
			  }
			  if(-1 == snprintf(a1,MAXLENWEIGHT,"%s",ax)){
				  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
			  }
			  checkone(a1);
			  checkone(b1);
	  ...
	  /*************************************/
		  ...
		  if (product(atmp,btmp,a1,b1)) {
			  char bx[100];  
			  if(t[l2][1]==TIN){ 
				  if(-1 == snprintf(bx,MAXLENWEIGHT,"X(%d,%ld)%s",SYNC,l1-1,b1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
			  }else{ 
				  if(-1 == snprintf(bx,MAXLENWEIGHT,"X(%d,%ld)%s",SYNC,l1,b1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
			  }
			  if(-1 == snprintf(b1,MAXLENWEIGHT,"%s",bx)){
				  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
			  }
			  checkone(b1);
			  checkone(a1);
	  ...
	  /******************************************/
		  ...
		  if (l1==0) {	    
			  if(-1 == snprintf(bcarlo,MAXLENWEIGHT,"X(%d,%ld)",FUNC,l2)){
				  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
			  }
			  strncat(bcarlo,btmp,MAXLENWEIGHT-1);
			  strncpy(btmp,bcarlo, MAXLENWEIGHT);
		  }
		  if (product(atmp,btmp,a1,b1)) {
			  int ft;
			  char ax[MAXLENWEIGHT];
			  ...
			  if (l1==0) {	
				  ...
				  if(-1 == snprintf(ax,MAXLENWEIGHT,"X(%d,%d)%s",TYPE,ft,a1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
				  if(-1 == snprintf(a1,MAXLENWEIGHT,"%s",ax)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  ...
				  ft=findType(f[l2].type);
				  if(-1 == snprintf(ax,MAXLENWEIGHT,"X(%d,%d)%s",TYPE,ft+1,a1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
				  if(-1 == snprintf(a1,MAXLENWEIGHT,"%s",ax)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  ...
				  if(-1 == snprintf(bx,MAXLENWEIGHT,"X(%d,%ld)%s",FUNC,l2,b1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
				  if(-1 == snprintf(b1,MAXLENWEIGHT,"%s",bx)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
	  ...
	  /*************************************/
		  ...
		  if (l2==0) {	    
			  if(-1 == snprintf(acarlo,MAXLENWEIGHT,"X(%d,%ld)",FUNC,l1)){
				  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
			  }
			  strncat(acarlo,atmp,MAXLENWEIGHT-1);
			  strncpy(atmp,acarlo,MAXLENWEIGHT);
		  }
		  if (product(atmp,btmp,a1,b1)) {
			  int ft;
			  char ax[MAXLENWEIGHT];
			  ...
			  if (l2==0) {	
				  ...
				  if(-1 == snprintf(bx,MAXLENWEIGHT,"X(%d,%d)%s",TYPE,ft,b1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
				  if(-1 == snprintf(b1,MAXLENWEIGHT,"%s",bx)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  ...
				  ft=findType(f[l1].type);
				  if(-1 == snprintf(ax,MAXLENWEIGHT,"X(%d,%ld)%s",FUNC,l1,a1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
				  if(-1 == snprintf(a1,MAXLENWEIGHT,"%s",ax)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  ...
				  if(-1 == snprintf(bx,MAXLENWEIGHT,"X(%d,%d)%s",TYPE,ft+1,b1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
				  if(-1 == snprintf(b1,MAXLENWEIGHT,"%s",bx)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
	  ...
	  /*************************************/
...
#ifdef WDEBUG
		  printf("%d) CONST-FUNC WAIT %ld, NARG %d\n",rank, f[l2].wait,f[l2].narg);
#endif
		  if ( f[l2].wait == f[l2].narg ) {
			  ...
			  if (product(atmp,btmp,a1,b1)) {
				  ...
				  //char ax[100];
				  //char bx[100];
				  if(-1 == snprintf(ax,MAXLENWEIGHT,"X(%d,%d)%s",KONST,kindex,a1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
				  if(-1 == snprintf(bx,MAXLENWEIGHT,"X(%d,%ld)%s",FUNC,l2,b1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
	  ...
	  /*************************************/
...
#ifdef WDEBUG
		  printf("%d) FUNC-CONST WAIT %ld, NARG %d\n",rank,f[l1].wait,f[l1].narg);
#endif
		  if ( f[l1].wait == f[l1].narg ) {
			  ...
			  if (product(atmp,btmp,a1,b1)) {
				  ...
				  //char ax[100];
				  //char bx[100];
				  if(-1 == snprintf(bx,MAXLENWEIGHT,"X(%d,%d)%s",KONST,kindex,b1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
				  if(-1 == snprintf(ax,MAXLENWEIGHT,"X(%d,%ld)%s",FUNC,l1,a1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
	  ...
	  /********************* Interazioni relative all'if_then_else ***********/
		  ...
		  if (product(atmp,btmp,a1,b1)) {
			  ...
			  //char ax[100];  
			  if(k[l2][1]==0){
				  if(-1 == snprintf(ax,MAXLENWEIGHT,"X(%d,%d)%s",ITE,2,a1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
			  }else{
				  if(-1 == snprintf(ax,MAXLENWEIGHT,"X(%d,%d)%s",ITE,1,a1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
			  }
	  ...
	  /*************************************/
		  ...
		  if (product(atmp,btmp,a1,b1)) {
			  ...
			  //char bx[100];  
			  if(k[l1][1]==0){
				  if(-1 == snprintf(bx,MAXLENWEIGHT,"X(%d,%d)%s",ITE,2,b1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
			  }else {
				  if(-1 == snprintf(bx,MAXLENWEIGHT,"X(%d,%d)%s",ITE,1,b1)){
					  fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
				  }
			  }
			  if(-1 == snprintf(b1,MAXLENWEIGHT,"%s",bx)){
...
/*************************************/
	...
	if (l2==0&&l1==0) {	    
		char acarlo[MAXLENWEIGHT];
		if(-1 == snprintf(acarlo,MAXLENWEIGHT,"X(%d,%d)",ITE,0)){
			fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
		}
		strncat(acarlo,atmp,MAXLENWEIGHT-1);
		strncpy(atmp,acarlo,MAXLENWEIGHT);
		if (product(atmp,btmp,a1,b1)) {
			...
			char ax[MAXLENWEIGHT];
			//char ax[100];
			if(-1 == snprintf(ax,MAXLENWEIGHT,"X(%d,%ld)%s",ITE,l1,a1)){
				fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
			}
...
/*************************************/
	...
	if (l1==0&&l2==0) {	    
		char bcarlo[MAXLENWEIGHT];
		if(-1 == snprintf(bcarlo,MAXLENWEIGHT,"X(%d,%d)",ITE,0)){
			fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
		}
		strncat(bcarlo,btmp,MAXLENWEIGHT-1);
		strncpy(btmp,bcarlo,MAXLENWEIGHT);
	...
	if (product(atmp,btmp,a1,b1)) {
		if (l1==0&&l2==0) { return 31;}	
			...
			char bx[MAXLENWEIGHT];
			//char bx[100];  
			if(-1 == snprintf(bx,MAXLENWEIGHT,"X(%d,%ld)%s",ITE,l2,b1)){
				fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
			}
