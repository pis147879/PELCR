	...
	edge *e;
{
	...
	while(NewWeight != NULL)
	{
		...
		switch (Type)
		{
			...
			case 1:
				if(Index)
					ret=snprintf(symboliclabel,MAXLENWEIGHT," NATout");
				else
					ret=snprintf(symboliclabel,MAXLENWEIGHT," NATin");
				break;

				/*  KONST */
			case 2:
				ret=snprintf(symboliclabel,MAXLENWEIGHT," (%lld)",k[Index][1]);
				break;

				/*  FUNC */
			case 3:
				ret=snprintf(symboliclabel,MAXLENWEIGHT," %s",f_db[f[Index].fun_id].name);
				break;

				/*  SYNC */
			case 4:
				ret=snprintf(symboliclabel,MAXLENWEIGHT," SYNC(%d)",Index);
				break;

				/*  ARG */
			case 5:
				ret=snprintf(symboliclabel,MAXLENWEIGHT," ARG(%d)",Index);
				break;

				/*  ITE */
			case 6:
				ret=snprintf(symboliclabel,MAXLENWEIGHT," ITE(%d)",Index);
				break;
		...
		if(ret == -1){
			fprintf(stderr,"[WARN] [%s][%d] Truncated output in switch case %d \n",__FILE__, __LINE__, Type);
		}
...
#endif
	if(-1 == snprintf(name,MAXNAMELEN,"%s%5d%5ld%3ld.gml",outfile,rank,loops,sq)){
	...
	int sq;
{
	...
	OUTPUTFILE
	{
		...
		if(-1 == snprintf(name,MAXNAMELEN,"%s%d.%ld.%d.gml",outfile,rank,loops,sq)){
			fprintf(stderr,"[WARN][%s][%d] Truncated output\n",__FILE__,__LINE__);
		}
		if((writefile= fopen(name,"w"))==NULL)
		{
