...
void
writenode(FILE *wstream, node *p) {
#ifdef _DEBUG
	DEBUG_PRINT {
		fprintf(logfile, "(%d) WRITENODE (function writenode) %p\n", rank, (void *)p);
	}
#endif
	...
	} else if (((p->left.length == 1) && (p->left.eot == 1)) && ((p->right.length == 1) && (p->right.eot == 1))) {
...
void
writedge(FILE *wstream, edge *e, node *p, int c) {
#ifdef _DEBUG
	DEBUG_PRINT {
...
#endif
	if (p->printed != pflag) {
		printf("(%d) ERROR TARGET NODE NOT YET PRINTED \n", rank);
	}
	...
	switch (c) {
		case 0: {
			fprintf(wstream, " graphics [ fill \"#aa0000\" ");
		} break;
		case 1: {
			fprintf(wstream, " graphics [ fill  \"#0000aa\" ");
		} break;
			/*    case 2 : { fprintf(wstream," graphics [ fill  \"#00aa00\" ] \n") ;} break;*/
		default: {
			float col;
			col = 255 - 255 * (float)(c - 2) / (float)MINPRIORITY;
			TRACING printf("-----> %2.2x %d \n", (int)col, (int)col);
			fprintf(wstream, " graphics [ fill  \"#00%2.2x00\" ", (int)col);
		} break;
	}
...
void
mmwrite(FILE *wstream, struct messaggio *mm, int priority) {
...
#ifdef _DEBUG
				DEBUG_PRINT {
					fprintf(logfile, "(%d) WRITENODE (function mmwrite) %p\n", rank, (void *)target);
				}
#endif
...
int
write_buffer(FILE *wstream, struct mbuffer *b, int priority) {
	int i;
#ifdef _DEBUG
	DEBUG_PRINT {
...
void
Print(graph g, struct mbuffer *b, long sq) {
	node *P;
	edge *e;
	int i;
	...
	for (i = 0; P != NULL; i++) {
		...
		//		DEBUG_PRINT { printf("(%d) WRITENODE (function Print) node print flag (after writenode) :
		//%d\n",rank,P->printed);}
		P = P->nextpuit;
		//		DEBUG_PRINT { if (P!=NULL) printf("(%d) WRITENODE (function Print) node print flag (after writenode) :
		//%d\n",rank,P->printed);}
	};
