...
termGraph *
BuildF1Arg(char *namef, termGraph *arg) {
...
termGraph *
BuildXF1Arg(char *namef, termGraph *arg) {
	termGraph *newT = (termGraph *)malloc(sizeof(termGraph));
	node *root = (node *)malloc(sizeof(node));
	node *tmp = (node *)malloc(sizeof(node));
	node *tmp2 = (node *)malloc(sizeof(node));
...
termGraph *
BuildF2Arg(char *namef, termGraph *arg1, termGraph *arg2) {
...
termGraph *
BuildXF2Arg(char *namef, termGraph *arg1, termGraph *arg2) {
...
void
CallXF1ArgChar(char *namef, char *arg) {
	USERTYPE (*fun)(char *);
...
void
CallXF1ArgUser(char *namef, char *arg) {
...
int
XAddEntryInf(char *funname, int narg) {
...
termGraph *
BuildITE(termGraph *arg1, termGraph *arg2, termGraph *arg3) {
	...
	{
		edge *tmpe;
		for (tmpe = ((arg3->root)->left).vector; tmpe != NULL; tmpe = tmpe->vector)
			tmpe->sto = IN;
		for (tmpe = ((arg3->root)->right).vector; tmpe != NULL; tmpe = tmpe->vector)
			tmpe->sto = IN;
	}
...
/* ANTO */
int
AddEntryInk(int type, USERTYPE val) {
	/* ANTO */
...
edge *
AddAnEdge(edge *old, char *peso, node *sorgente, int lato) {
...
termGraph *
BuildLet(char *var, termGraph *t1, termGraph *t2) {
	termGraph *t;
...
termGraph *
BuildLambda(char *var, termGraph *ter) {
...
termGraph *
BuildRec(char *xname, termGraph *M) {
...
table *
RemoveFromList(table *list, char *var) {
	table *tmp = list;
	table *tmpPrev;
...
termGraph *
BuildApplication(termGraph *t1, termGraph *t2) {
...
termGraph *
MergeNodes(termGraph *new, termGraph *t1, termGraph *t2) {
...
void
PutSymbol(char *symbol, termGraph *t) {
	symTbl *tmp;
	int trovato = 0;

	tmp = symbolTable;
	while ((tmp != NULL) && (trovato == 0)) {
		if (strcmp(tmp->symName, symbol) == 0)
			trovato = 1;
		else
			tmp = tmp->next;
	}
...
void
InitializeIncoming(termGraph *t) {
	node *nodo;
	edge *arco;
